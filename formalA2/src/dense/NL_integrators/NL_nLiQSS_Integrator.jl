...
function integrate(Al::QSSAlgorithm{:nliqss,O},CommonqssData::CommonQSS_data{0},liqssdata::LiQSS_data{O,false},specialLiqssData::SpecialLiqssQSS_data, odep::NLODEProblem{PRTYPE,T,0,0,CS},f::Function,jac::Function,SD::Function,exactA::Function) where {PRTYPE,CS,O,T}
  ...
  d=[0.0]# this is a dummy var used in nupdateQ and simulUpdate because in the discrete world exactA needs d, this is better than creating new nupdateQ and simulUpdate functions
 ...
 #@show f
  ...
  for k = 1:O # compute initial derivatives for x and q (similar to a recursive way )
   ...
   for i = 1:T
    ...
    nupdateQ(Val(O),i,x,q,quantum,exactA,d,cacheA,dxaux,qaux,tx,tq,initTime,ft,nextStateTime) 
  end
 # for i = 1:T
    # clearCache(taylorOpsCache,Val(CS),Val(O));f(i,q,t,taylorOpsCache);
...
#=     if (simt>ft/2 || totalSteps==40000) && printonce==0
    ...
    if sch[3] == :ST_STATE
        ...
        for b in (jac(index)  )    # update Qb : to be used to calculate exacte Aindexb...move below nupdateQ
          elapsedq = simt - tq[b] ;
          if elapsedq>0 integrateState(Val(O-1),q[b],elapsedq);tq[b]=simt end
        end
        firstguess=nupdateQ(Val(O),index,x,q,quantum,exactA,d,cacheA,dxaux,qaux,tx,tq,simt,ft,nextStateTime) ;tq[index] = simt   
        ...
        for j in SD(index)
          ...
          if j!=index && aij*aji!=0.0
              ...
              if nisCycle_and_simulUpdate(cacheRootsi,cacheRootsj,acceptedi,acceptedj,aij,aji,respp,pp,trackSimul,Val(O),index,j,dirI,firstguess,x,q,quantum,exactA,d,cacheA,dxaux,qaux,tx,tq,simt,ft)
                simulStepCount+=1
   ...
   #=  println("nmliqss intgrator under input, index= $index, totalsteps= $totalSteps")
    inputSteps+=1
      ...
      #for k = 1:O q[index].coeffs[k] = x[index].coeffs[k] end; tq[index] = simt 
        notneeded=nupdateQ(Val(O),index,x,q,quantum,exactA,cacheA,dxaux,qaux,tx,tq,simt,ft,nextStateTime) ;tq[index] = simt 
        for b in jac(index) 
          elapsedq = simt - tq[b];if elapsedq>0 integrateState(Val(O-1),q[b],elapsedq);tq[b]=simt end
        end
