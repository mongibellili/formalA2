...
function integrate(Al::QSSAlgorithm{:mliqss,O},CommonqssData::CommonQSS_data{0},liqssdata::LiQSS_data{O,false},specialLiqssData::SpecialLiqssQSS_data, odep::NLODEProblem{PRTYPE,T,0,0,CS},f::Function,jac::Function,SD::Function,exactA::Function) where {PRTYPE,CS,O,T}
...
function integrate(Al::QSSAlgorithm{:mliqssBounds,O},CommonqssData::CommonQSS_data{0},liqssdata::LiQSS_data{O,false},specialLiqssData::SpecialLiqssQSS_data, odep::NLODEProblem{PRTYPE,T,0,0,CS},f::Function,jac::Function,SD::Function,exactA::Function) where {PRTYPE,CS,O,T}
 ...
 createSol(Val(T),Val(O),savedTimes,savedVars, "nmliqssbounds$O",string(odep.prname),absQ,totalSteps,simulStepCount,0,numSteps,ft)
     # change this to function /constrcutor...remember it is bad to access structs (objects) directly
