...
function isCycle_and_simulUpdate(::Val{2},index::Int,j::Int, x::Vector{Taylor0{Float64}},q::Vector{Taylor0{Float64}}, quantum::Vector{Float64},a::MVector{T,MVector{T,Float64}},u::MVector{T,MVector{T,MVector{O,Float64}}},qaux::MVector{T,MVector{O,Float64}},olddx::MVector{T,MVector{O,Float64}},olddxSpec::MVector{T,MVector{O,Float64}},tx::MVector{T,Float64},tq::MVector{T,Float64},tu::MVector{T,Float64},simt::Float64,ft::Float64)where{T,O}
  ...
  u[j][j][2]=xj2-ajj*qj1##########-----------------------
  #= if templ!=u[j][j][2]
    println("old ujj2  ",templ)
    println("ujj2 updated= ",u[j][j][2])
 ...
 #u[j][index][2]=u[j][j][2]-ajj*qaux[index][1] # from article p20 line25 more cycles ...shaky with no bumps
  ...
  iscycle=false
    ...
    uij2=u[index][j][2]
 #=    if index==1
      uji=1.2;uji2=0.0;uji3=0.0
      uij=0.2;uij2=0.0;uij3=0.0
    
    else
      uji=0.2;uji2=0.0;uji3=0.0
      uij=1.2;uij2=0.0;uij3=0.0
    end
    
    u[j][index][1]=uji;u[j][index][2]=uji2;
    u[index][j][1]=uij;u[index][j][2]=uij2; =#
...
function isCycle_and_simulUpdate(::Val{3},index::Int,j::Int, x::Vector{Taylor0{Float64}},q::Vector{Taylor0{Float64}}, quantum::Vector{Float64},a::MVector{T,MVector{T,Float64}},u::MVector{T,MVector{T,MVector{O,Float64}}},qaux::MVector{T,MVector{O,Float64}},olddx::MVector{T,MVector{O,Float64}},olddxSpec::MVector{T,MVector{O,Float64}},tx::MVector{T,Float64},tq::MVector{T,Float64},tu::MVector{T,Float64},simt::Float64,ft::Float64)where{T,O}
