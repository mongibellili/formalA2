...
for i = 1:T
  ...
  clearCache(taylorOpsCache,cacheSize);f(i,q,d,t,taylorOpsCache);#@show taylorOpsCache
...
while simt < ft && printcount < 500000 
 # @show abort
  ...
  if sch[3] == :ST_STATE
   # if simt>=3.09e-5
   #=  println("before intgrateX and update q")
   ...
   # if simt>=3.09e-5
  #=   println("after intgrateX and update q")
  ...
  #  end
    for i = 1:length(SD[index])
      ...
      if j != 0      
        ...
        #= println("after intgrateX intgrateQ inside dependency")  
        @show j
        @show x
        @show q    =# 
  ...
  elseif sch[3] == :ST_INPUT  # time of change has come to a state var that does not depend on anything...no one will give you a chance to change but yourself    
   ...
   #if 0.4>simt > 0.31
     # println("$index  ------------------------------nextInputTime from input= ",nextInputTime);@show x 
    # end 
    for i = 1:length(SD[index])
      j = SD[index][i] 
      if j != 0      
