...
function consructJacobiansSystemA(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
...
function consructJacobiansSystemB(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in [-0.1,0.1]
           for a22 in coeffs
               if a11+a22>0
...
function consructJacobiansSystemD(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in coeffs
               if a11+a22>0
                   for a12 in coeffs
                       for a21 in coeffs
                             if a11*a22!=a12*a21 # to have A invertible...not needed since β<0 & Δ<0 is false
                               if (a11-a22)^2+4*(a12*a21) <0 # this is just to have real solutions
                                   A=[a11 a12;a21 a22]
                                   push!(allJacsys,A)
                               end
...
#=    coefs=[-0.75,-12.37,2.0]
   ...
   us=1;ul=20
   counter=12960
   writeProblemstoFile("B2",coefs,posCoefs,negCoefs,consructJacobiansSystemB,initConds,us,ul,counter)
end

#createRealProblem()

function createComplexProblem()
    #=   coefs=[-0.45,-0.56,-0.98,-1.09,-1.28,-1.86,-2.12,-2.78,-3.37,-4.27,-6.32,-8.5,-10.5,-11.6,-13.48,-15.6,-17.05,-19.87,-22.66,0.46,0.55,0.97,1.05,1.22,1.83,2.19,2.75,3.34,4.22,6.32,7.64,8.6,10.6,12.6,14.05,16.87,18.66,21.66,23.66]
     posCoefs=[0.11,0.12,0.13,0.14,0.15,0.16,0.17,0.18,0.19,0.2,0.21,0.22,0.23,0.24,0.25,0.26,0.27,0.28,0.29,0.3,0.31,0.32,0.33,0.34,0.35,0.36,0.37,0.38,0.39,0.46,0.55,0.97,1.05,1.22,1.83,2.19,2.75,3.34,4.22,6.32,7.64,8.6,10.6,12.6,14.05,16.87,18.66,21.66,23.66]
     negCoefs=[-0.11,-0.12,-0.13,-0.14,-0.15,-0.16,-0.17,-0.18,-0.19,-0.2,-0.21,-0.22,-0.23,-0.24,-0.25,-0.26,-0.27,-0.28,-0.29,-0.3,-0.31,-0.32,-0.33,-0.34,-0.35,-0.36,-0.37,-0.38,-0.39,-0.45,-0.56,-0.98,-1.09,-1.28,-1.86,-2.12,-2.78,-3.37,-4.27,-6.32,-8.5,-10.5,-11.6,-13.48,-15.6,-17.05,-19.87,-22.66]
     =#
     coefs=[-0.1,-0.75,-2.0,-12.37,-21.0,0.1,0.75,2.0,12.64,27.19]
     posCoefs=[0.2,2.36,7.21,15.4,29.22]
     negCoefs=[-0.956,-2.65,-5.3,-8.09,-12.65,-18.958]
  #=    coefs=[-0.75,-12.37,2.0]
     posCoefs=[0.2]
     ...
     us=1;ul=20
     counter=0
     writeComplexProblemstoFile("D",coefs,posCoefs,negCoefs,consructJacobiansSystemD,initConds,us,ul,counter)
  end
  
  createComplexProblem()