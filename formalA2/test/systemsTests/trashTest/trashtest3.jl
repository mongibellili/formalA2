...
#= posCoeffs=NTuple{7,Float64}((29160.956861496, 67.56376290717117, 0.014452560693316113,56.2,1250.12,565.21,0.002))
coeff=posCoeffs[2:end]
@show coeff
@show typeof(coeff) =#
...
#= function phi(h::Float64)
  -sin(h)
  #h*h*h*h-14.0*h*h*h+60.0*h*h-70.0*h
 end
 function goldenSearch(a0::Float64,b0::Float64,N::Int)
  ρ=0.382
  a1=a0;b1=b0
  for k=1:N
    a0=a1;b0=b1
...
#@show goldenSearch(0.0,3.0,10)

@show phi(3.008197029541609) =#

function consructJacobiansSystemF(posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
