...
#= function consructJacobiansSystemC(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
     allJacsys=Vector{ Array{Float64, 2}}()
     for a11 in negCoeffs
        #if a11<0
            for a22 in negCoeffs
                #if a22<0
                    for a12 in coeffs
                        for a21 in coeffs
...
#= function consructJacobiansSystemF(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in negCoeffs
       #if a11<0
           for a22 in negCoeffs
               #if a22<0
                   for a12 in negCoeffs
                       for a21 in posCoeffs
                           #if a12*a21<0#different signs No need since posCoeffs are >0 and 
                               if (a11-a22)^2>4*abs(a12*a21)
                                   A=[a11 a12;a21 a22]
                                   push!(allJacsys,A)
                               end
                           #end
  ...
  function writeProblemstoFile(prbName,coefs,negCoefs,sysFunc,initConds,us,ul,counter)
    jacs=sysFunc(coefs,negCoefs)
    inputs=[[us,us],[-us,-us],[-us,us],[us,ul],[-us,-ul],[-us,ul],[us,-ul],[ul,ul],[-ul,-ul],[-ul,ul]]
   # inputs=[[us,us]]
   @show jacs
    #counter=0
    
    path="./formalA2/test/type$(prbName).jl" #default path
...
#= function consructJacobiansSystemG(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in negCoeffs
               if a11+a22<0
...
#= function consructJacobiansSystemH(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in negCoeffs
               if a11+a22<0
                   for a12 in coeffs
                       for a21 in coeffs
                           if (a11*a22)<(a12*a21)
                               if (a11-a22)^2>4*abs(a12*a21)  # this is just to have real solutions
                                   A=[a11 a12;a21 a22]
                                   push!(allJacsys,A)
                               end
                           end
...
function consructJacobiansSystemI(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in negCoeffs
               if a11+a22>0
...
function consructJacobiansSystemJ(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in negCoeffs
               if a11+a22>0
...
function consructJacobiansSystemK(coeffs:: Vector{Float64},posCoeffs:: Vector{Float64},negCoeffs:: Vector{Float64})
    allJacsys=Vector{ Array{Float64, 2}}()
    for a11 in coeffs
           for a22 in coeffs
            if a11!=a22
               if a11+a22>0
                   for a12 in posCoeffs
                       for a21 in negCoeffs
                           if (a11*a22)>(a12*a21)
...
#= function createProblemC()
     #= coefs=[-0.5,-2.0,-5.5,-10.0,-25.0,0.5,2.0,5.5,10.0,25.0]
    posCoefs=[0.5,2.0,5.5,10.0,25.0]
    negCoefs=[-0.5,-2.0,-5.5,-10.0,-25.0] =#
    coefs=[-20.0,-1.0,1.0,20.0]
    negCoefs=[-21.0,-1.1]
    initConds=[-1.0,-2.0]
    us=1;ul=20
    counter=220 # to add problems and not override previously saved problems
    writeProblemstoFile("C",coefs,posCoefs,negCoefs,consructJacobiansSystemC,initConds,us,ul,counter)
end =#

#createProblemC()

#= function createProblemF()
    #coefs=[-0.5,-2.0,-5.5,-10.0,-25.0,0.5,2.0,5.5,10.0,25.0]
   posCoefs=[0.5,2.0,5.5,10.0,25.0]
   negCoefs=[-0.5,-2.0,-5.5,-10.0,-25.0]
   #= coefs=[1.0,20.0]
   ...
   us=1;ul=20
   counter=60
   writeProblemstoFile("F",coeffs,posCoefs,negCoefs,consructJacobiansSystemF,initConds,us,ul,counter)
end =#

#createProblemF()

#= function createProblemG()
    coefs=[-0.5,-2.0,-5.5,-10.0,-25.0,0.5,2.0,5.5,10.0,25.0]
   posCoefs=[0.5,2.0,5.5,10.0,25.0]
   negCoefs=[-0.5,-2.0,-5.5,-10.0,-25.0]
   #= coefs=[1.0,20.0]
   ...
   us=1;ul=20
   counter=0
   writeProblemstoFile("G",coefs,posCoefs,negCoefs,consructJacobiansSystemG,initConds,us,ul,counter)
end =#
...
#= function createProblemH()
    coefs=[-0.75,-2.0,-12.37,-21.0,0.75,2.0,12.64,27.19]
   posCoefs=[0.2,15.4,29.22]
   negCoefs=[-0.956,-8.09,-18.958]
   #= coefs=[1.0,20.0]
   negCoefs=[-21.0,-1.1] =#
   initConds=[-1.0,-2.0]
   us=1;ul=20
   counter=140
   writeProblemstoFile("H",coefs,posCoefs,negCoefs,consructJacobiansSystemH,initConds,us,ul,counter)
end =#

#createProblemH()


#= function createProblemI()
    coefs=[-0.45,-1.78,-3.37,-8.5,0.44,1.6,3.64,7.19,10.07]
   posCoefs=[0.3,4.4,9.22]
   negCoefs=[-0.23,-1.35,-5.09,-11.958]
   #= coefs=[1.0,20.0]
   negCoefs=[-21.0,-1.1] =#
   initConds=[-1.0,-2.0]
   us=1;ul=20
   counter=36
   writeProblemstoFile("I",coefs,posCoefs,negCoefs,consructJacobiansSystemI,initConds,us,ul,counter)
end

createProblemI() =#

#= function createProblemJ()
    coefs=[-0.75,-2.0,-12.37,-21.0,0.75,2.0,12.64,27.19]
   posCoefs=[0.2,15.4,29.22]
   negCoefs=[-0.956,-8.09,-18.958]
   #= coefs=[1.0,20.0]
   ...
   us=1;ul=20
   counter=0
   writeProblemstoFile("J",coefs,posCoefs,negCoefs,consructJacobiansSystemJ,initConds,us,ul,counter)
end
...
#=    coefs=[-0.75,-12.37,2.0]
   ...
   us=1;ul=20
   counter=0
   writeComplexProblemstoFile("K",coefs,posCoefs,negCoefs,consructJacobiansSystemK,initConds,us,ul,counter)
end
...
#= function writeoneProblem(prbName)
    jacs=[[-0.5 2.0;10.0 -25.0]]
    #inputs=[[us,us],[-us,-us],[-us,us],[us,ul],[-us,-ul],[-us,ul],[us,-ul],[ul,ul],[-ul,-ul],[-ul,ul]]
    initConds=[10.0,-10.0]
    counter=0
   us=-10;ul=-20
